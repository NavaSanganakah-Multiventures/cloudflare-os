...
export class GitHubApi {
  ...
  async getDirectory(owner: string, repo: string, path: string, ref?: string): Promise<any> {
    const query = ref ? `?ref=${encodeURIComponent(ref)}` : "";
    return (await this.#request(
      "GET",
      `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/contents/${path.split("/").map(encodeURIComponent).join("/")}${query}`,
  ...
  async createOrUpdateFile(
    ...
    path: string,
    message: string,
    content: string, // this is raw content, will base64 it
    branch?: string,
    sha?: string,
  ): Promise<any> {
    let encodedContent: string;
    try {
      encodedContent = typeof Buffer !== "undefined"
        ? Buffer.from(content, "utf8").toString("base64")
        : btoa(encodeURIComponent(content).replace(/%([0-9A-F]{2})/g, (match, p1) => String.fromCharCode(parseInt(p1, 16))));
    } catch {
      encodedContent = typeof btoa !== "undefined" ? btoa(content) : "";
    }
  ...
  async createBranch(owner: string, repo: string, ref: string, sha: string): Promise<any> {
    return (await this.#request(
      "POST",
  ...
  async getBranch(owner: string, repo: string, branchName: string): Promise<{ ref: string; sha: string }> {
